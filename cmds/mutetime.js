    ...
    if(cmd == 'set'){
        if(!time){
            ...
            db.get(`SELECT * FROM mutetime WHERE guild_Id = ${guild}`).then(row => {
                if(!row){
                    ...
                    message.channel.send(`The time has been set to **${ms(ms(time))}**.`);
                } else {
                    db.run(`UPDATE mutetime SET waktu='${time}' WHERE guild_Id = ${guild}`);
                    message.channel.send(`The time has been change to **${ms(ms(time))}**.`);
                }
            });
        }
    } else if (cmd == 'info') {
        ...
        db.get(`SELECT * FROM mutetime WHERE guild_Id = ${guild}`).then(row => {
            if(!row) {
                ...
                message.channel.send(`The time has been deleted!.`);
            }
        });
    }
}
